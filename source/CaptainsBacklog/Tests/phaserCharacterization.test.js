describe("phasers", function() {
    ...
    beforeEach(function() {
        game = new Game();
        energyBefore = game.weapons.phasers.ammo;
        ui = new UserInterface("phasers");
        spyOn(ui, "writeLine");
    });

    it("should complain with insufficient energy when not available", function() {
        // given
        ui.commandParameter = game.weapons.phasers.ammo + 1;
    ...
    describe("when Klingon out of range", function() {
        ...
        beforeEach(function() {
            outOfRange = game.weapons.phasers.maxRange + 1;
        ...
        it("still subtracts the energy", function() {
            expect(game.weapons.phasers.ammo).toBe(energyBefore - energyToFire);
        });
    });
    ...
    describe("when Klingon destroyed by sufficient strike", function() {
        ...
        it("subtracts energy", function() {
            expect(game.weapons.phasers.ammo).toBe(energyBefore - 1000);
        });
    ...
    describe("when damaging Klingon", function() {
        ...
        it("subtracts energy", function() {
            expect(game.weapons.phasers.ammo).toBe(energyBefore - 50);
        });
    });
    ...
    describe("a defect when firing zero", function() {
        ...
        it("mistakenly doesn't subtract that one unit", function() {
            expect(game.weapons.phasers.ammo).toBe(energyBefore);
        });
    });
